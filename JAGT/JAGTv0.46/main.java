...
public class main {
	...
	public static String divider(int ax, int ay, int bx, int by, int[] ratio) {
		...
		String fin = "Coordinates of line-splitting point: (" + (double)Math.round(fx * 1000d)/1000d + ", " + (double)Math.round(fy * 1000d)/1000d + ")"; 
		return(fin);
	}
	...
	public static String cequation(int cx, int cy, int pointx, int pointy, int type){
		...
		if (type == 0){
			fin = "Eqaution of the circle: (x - " + cx + ")" + "\u00B2" + " + (y - " + cy + ")" + "\u00B2" + " = " + pointx + "\u00B2" + " (" + (int)pointx * pointx + ")"; 
		}
		//For if a point is given
		else{
			double r = Double.parseDouble(distance(cx, cy, pointx, pointy, 0));
			fin = "Eqaution of the circle: (x - " + cx + ")" + "\u00B2" + " + (y - " + cy + ")" + "\u00B2" + " = " + (double)Math.round(r * 1000d)/1000d + "\u00B2" + " (" + (int)Math.round((r * r) * 1d)/1d + ")";
		}
		return fin;
	}
	...
	public static String point_slope(double[] crds, double[] _slope, boolean isDecimal) {
		...
		if (isDecimal) {
			...
			f = "y = " + (double)Math.round(slopeAsDecimal * 1000d)/1000d + "x + " + (double)Math.round(intercept * 1000d)/1000d;
		}
		else {
			f = "y = " + pslope[0] + "/" + pslope[1] + "x + " + (double)Math.round(intercept * 1000d)/1000d;
		}
		return f;
	}
	...
	//Used by a lot of stuff; finds slope of line formed by coords
